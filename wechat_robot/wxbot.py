...
# 向好友发送消息，name 为好友的备注名或者好友微信号， isfile为 False 时 word 为消息，isfile 为 True 时 word 为文件路径(此时向好友发送文件里的每一行)
    ...
    def run(self):
        ...
        self.gen_qr_code(os.path.join(self.temp_pwd, 'qr.png'))
    ...
    def gen_qr_code(self, qr_file_path):
        ...
        if self.conf['qr'] == 'png':
            ...
            if sys.platform.find('darwin') >= 0:
                ...
                subprocess.call(['open', os.path.join(os.getcwd(), 'temp', qr_file_path)])
            elif sys.platform.find('linux') >= 0:
                subprocess.call(['xdg-open', os.path.join(os.getcwd(), 'temp', qr_file_path)])
                # subprocess.call(['xdg-open', qr_file_path])
            else:
                # os.startfile(qr_file_path)
                os.startfile(os.path.join(os.getcwd(), 'temp', qr_file_path))
